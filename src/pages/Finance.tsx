...
const ISSUES = [
  { title: 'CPL вырос ×2.3 в Meta', severity: 'high', impact: '-₽480 000/мес', fix: 'Пересмотреть таргетинг и креативы' },
  { title: 'LTV упал на 18%', severity: 'high', impact: '-₽320 000/мес', fix: 'Запустить retention-программу' },
  { title: 'Операционные расходы +31%', severity: 'medium', impact: '-₽260 000/мес', fix: 'Аудит подрядчиков и SaaS-подписок' },
]

const SEV_COLOR: Record<string, string> = { high: '#ef4444', medium: '#f59e0b', low: '#10b981' }
const SEV_LABEL: Record<string, string> = { high: 'Критично', medium: 'Важно', low: 'Низкое' }
      ...
      <div style={{ marginBottom: 'var(--space-8)' }}>
        ...
        >
          ...
          >
            ...
            <span style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-sm)', fontWeight: 600, color: 'white' }}>
      ...
      {analyzed && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--space-4)', marginBottom: 'var(--space-6)' }}>
            {[
              { label: 'Выручка (июл)', value: '₽2.9М', delta: '-₽310К vs июн', up: false },
              { label: 'Прибыль', value: '₽493К', delta: '-26% vs июн', up: false },
              { label: 'Маржа', value: '17%', delta: 'Was 28% в Q1', up: false },
              { label: 'CAC', value: '₽4 200', delta: '+82% за квартал', up: false },
            ].map((k) => (
              <div key={k.label} style={{ background: 'var(--color-bg-surface)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-xl)', padding: 'var(--space-5)' }}>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', margin: '0 0 var(--space-2) 0', fontWeight: 500 }}>{k.label}</p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  {k.up ? <TrendingUp size={12} color="var(--color-accent-success)" /> : <TrendingDown size={12} color="var(--color-accent-danger)" />}
                  <span style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', color: k.up ? 'var(--color-accent-success)' : 'var(--color-accent-danger)' }}>{k.delta}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Charts */}
          <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 'var(--space-5)', marginBottom: 'var(--space-6)' }}>
            {/* Revenue chart */}
            <div style={{ background: 'var(--color-bg-surface)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-xl)', padding: 'var(--space-6)' }}>
              <h3 style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-base)', fontWeight: 600, color: 'var(--color-text-primary)', margin: '0 0 var(--space-5) 0' }}>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={REVENUE_DATA}>
                  ...
                  <XAxis dataKey="month" tick={{ fontFamily: 'var(--font-mono)', fontSize: 11, fill: '#555a6b' }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontFamily: 'var(--font-mono)', fontSize: 11, fill: '#555a6b' }} axisLine={false} tickLine={false} />
                  <Tooltip content={<CustomTooltip />} />
                  <Area type="monotone" dataKey="revenue" name="Выручка" stroke="#5b6ef5" fill="url(#revGrad)" strokeWidth={2} />
                  <Area type="monotone" dataKey="profit" name="Прибыль" stroke="#ef4444" fill="url(#profitGrad)" strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            <div style={{ background: 'var(--color-bg-surface)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-xl)', padding: 'var(--space-6)' }}>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={EXPENSE_DATA} barGap={4}>
                  ...
                  <XAxis dataKey="cat" tick={{ fontFamily: 'var(--font-mono)', fontSize: 10, fill: '#555a6b' }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontFamily: 'var(--font-mono)', fontSize: 10, fill: '#555a6b' }} axisLine={false} tickLine={false} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="prev" name="Прошлый" fill="rgba(91,110,245,0.2)" radius={[4, 4, 0, 0]} />
          ...
          {/* Issues */}
          <div>
            <h3 style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-base)', fontWeight: 600, color: 'var(--color-text-primary)', margin: '0 0 var(--space-4) 0' }}>
              Выявленные проблемы
            </h3>
              ...
              {ISSUES.map((issue) => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                    ...
                    <div>
                      <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--color-text-primary)', margin: '0 0 2px 0' }}>{issue.title}</p>
                      <p style={{ fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', margin: 0 }}>{issue.fix}</p>
                    </div>
                  </div>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', fontWeight: 600, color: SEV_COLOR[issue.severity], whiteSpace: 'nowrap' }}>
                    {issue.impact}
                  </span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: SEV_COLOR[issue.severity], background: `${SEV_COLOR[issue.severity]}15`, border: `1px solid ${SEV_COLOR[issue.severity]}25`, borderRadius: 'var(--radius-full)', padding: '2px 8px', whiteSpace: 'nowrap' }}>
