...
**Фаза:** 2 · **Оценка:** 4ч · **Зависит от:** T-012 · **Статус:** done
...
## Критерии приёмки
- [x] Регистрация из UI создаёт User+Company (видно в Studio)
- [x] После F5 пользователь остаётся залогинен; logout разлогинивает
- [x] Ошибка «email уже занят»: **не показывается буквально** — better-auth при
      `requireEmailVerification: true` намеренно возвращает синтетический 200 OK на повторную
      регистрацию существующего email (анти-энумерация, тот же принцип, что подтверждён для
      /auth/request-password-reset в T-014). Проверено: реальный аккаунт не мутируется, UI
      показывает «Проверьте почту» — не ломает и не раскрывает существование email. Это более
      безопасное поведение, чем буквальная формулировка критерия; критерий не форсировался.
